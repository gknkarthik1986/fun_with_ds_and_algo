...
public class BPlusTree<KEY extends Comparable<? super KEY>, VALUE> {
	...
	/**
	 * Insert at the root propagates down to the leaf level.
	 */
	public void insert(KEY key, VALUE value) {
		...
		if (result != null) {
			...
			IntermediateNode _root = new IntermediateNode();
			_root.num = 1;
	...
	public VALUE find(KEY key) {
		Node node = root;
		while (node instanceof BPlusTree.IntermediateNode) { // need to traverse down to the leaf
			IntermediateNode inner = (IntermediateNode) node;
	...
	class IntermediateNode extends Node {
		...
		public Split insert(KEY key, VALUE value) {
			 ...
			 * 
			 * Since we do a top-down split by the time we reach the current node we know 
			 * for a fact that the parent is NOT full and hence we should be able to push the 
			 * middle element to the parent.
			 */

			if (this.num == N) { // Split
				int mid = (N + 1) / 2;
				int sNum = this.num - mid;
				IntermediateNode sibling = new IntermediateNode();
				sibling.num = sNum;
				System.arraycopy(this.keys, mid, sibling.keys, 0, sNum);
